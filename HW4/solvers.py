...
def optimizeWithIP(x0, A, b, C, d, mu=1.0, rate=0.1, tol=1e-6, max_iter=1000):
    ...
    F[id_x] = np.dot(A.T, A.dot(x)-b) + np.dot(C.T, v)
    F[id_v] = np.dot(np.diag(s), v) - mu
    F[id_s] = s + C.dot(x) - d
    ...
    # *************************************************************************
    dF[id_x, id_x] = np.dot(A.T, A)
    dF[id_x, id_v] = C.T

    dF[id_v, id_v] = np.diag(s)
    dF[id_v, id_s] = np.diag(v)

    dF[id_s, id_s] = np.eye(k)
    dF[id_s, id_x] = C
    ...
    #
        ...
        print('{}{}{}{}'.format('iteration: ', iter_count, ', error: ', err))
        #
        iter_count += 1
